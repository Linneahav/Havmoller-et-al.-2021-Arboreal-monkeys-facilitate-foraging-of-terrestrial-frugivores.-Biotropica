...
PB$Treatment <- factor(PB$Treatment, 
  levels = c("Pre", "Control 1", "Control 2", "Experiment 1",
...
### Define the N(0,1) quantile giving the confidence interval width for 6 comparisons, 
...
### Make room for two-line string for Ex4. 
string.positions <- (n.treatments:1) - c(rep(0, length(c.rate)-1), 0.5)
...
### Make plot.
jpeg("playback_contrasts_plot2.jpg", height=6, width=8, units="in", res=750)
plot(x=c.rate, y=(n.treatments:1), bty="n", yaxt="n", xaxt="n", xlab="", ylab="", 
     xlim=c(xmin, xmax), type="n", ylim=c(0, n.treatments + 0.5))
segments(x0=c.rate - (z95*SE.treatment), y0=(n.treatments:1), x1=c.rate + (z95*SE.treatment), lwd=3,
         col="grey60")
segments(x0=c.rate - (z90*SE.treatment), y0=(n.treatments:1), x1=c.rate + (z90*SE.treatment), lwd=4,
         col="blue3")
segments(x0=c.rate - (z85*SE.treatment), y0=(n.treatments:1), x1=c.rate + (z85*SE.treatment), lwd=6,
         col="dodgerblue1")
points(x=c.rate, y=(n.treatments:1), pch=19, cex=2.0, col="grey20")
treatment.string <-c("C1: No sound", "C2: Cicadas", "Ex1: Fruit dropping", 
                     "Ex2: Non-feeding vocalizations", "Ex3: Feeding vocalizations",
                     "Ex4: Feeding vocalizations\n      + fruit dropping")
...
### The event rate for each experimental and control condition is compared to the baseline 
### (pre-treatment) event rate.
### The x-axis is on the natural log-scale, though the tick marks are labeled with relative rates. 
### Confidence interval widths are corrected for six comparisons: each treatment compared with baseline.
